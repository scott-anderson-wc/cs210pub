...
function unpackPublicKey()
{
 ...
 document.t.pq.value = s2hex(b2s(mod));
...
function RSAdoEncryption_sda()
{
 ...
 p = hex2s(document.sign.dec_digest.value);
 var pl = p.length;
...
function RSAdoDecryption_sda()
{
  if( ! KeysGenerated ) {
    ...
    var md = document.sign.digest.value;
    if( md.length % 2 != 0 ) {
        alert("hex encoded string should have an even length");
        return;
    }
    var in_b=s2b(hex2s(md));
